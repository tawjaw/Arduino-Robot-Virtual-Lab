...
import { AVRRunner } from './Hardware/Uno/execute';
import { formatTime, getMilliSecconds } from './Hardware/Uno/format-time';
import './index.css';
import { CPUPerformance } from './Hardware/Uno/cpu-performance';
    ...
    */
    ...
    if(value & D9bit) 
    {
      if(pin9BeginningTimeOfPulse == undefined || pin9State === 0 )
        {
          pin9BeginningTimeOfPulse = runner.cpu.cycles;
          pin9State = 1;
        }
    } 
    else
    {
      if(pin9State === 1)
      {
        ...
        pin9State = 0;
        leftMotorSpeed = toFixedNumber(getMilliSecconds((runner.cpu.cycles - pin9BeginningTimeOfPulse )/MHZ)-1.4, 1, 10);
    ...
    if(value & D10bit) 
    {
      if(pin10BeginningTimeOfPulse == undefined || pin10State === 0 )
        {
          pin10BeginningTimeOfPulse = runner.cpu.cycles;
          pin10State = 1;
        }
    } 
    else
    {
      if(pin10State === 1)
      {
        ...
        pin10State = 0;
        rightMotorSpeed = toFixedNumber(getMilliSecconds((runner.cpu.cycles - pin10BeginningTimeOfPulse )/MHZ)-1.4, 1, 10);
