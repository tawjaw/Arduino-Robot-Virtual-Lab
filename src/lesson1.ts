...
const arduinoContainer = document.querySelector<ArduinoIDEContainer>('#ide-container');
arduinoContainer?.addEventListener('_status-change', (e : CustomEvent)=> handleIDEStatusChange(e))
...
const robot = new Robots.Arduino.TwoServoRobot(canvas, serialOutputText, arduinoContainer, 'imgs/room-background.jpg'); 
...
try {
  ...
  if (result.hex) {
    if(arduinoContainer)
      arduinoContainer.status='on';
    if(compilerOutputText)
    ...
    if(arduinoContainer)
      arduinoContainer.status='off';
    if(compilerOutputText)
      compilerOutputText.textContent = result.stderr;
  }
}
 } catch (err) {
    simulationStatus='off';
